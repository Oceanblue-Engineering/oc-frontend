...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  const [supplierShowDropdown, setSupplierShowDropdown] = useState(false);
  ...
  const selectedSuppliers = formData.suppliers || [];

  const toggleSupplier = (supplierId: string) => {
    const updated = selectedSuppliers.includes(supplierId)
      ? selectedSuppliers.filter(id => id !== supplierId)
      : [...selectedSuppliers, supplierId];
    updateFormData({ suppliers: updated });
  };
      ...
      <div className="bg-white p-6 rounded-lg w-full max-w-2xl my-8 max-h-[90vh] overflow-y-auto">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <div className="relative">
              <div 
                className="w-full border rounded p-2 flex justify-between items-center cursor-pointer bg-white"
                onClick={() => setSupplierShowDropdown(!supplierShowDropdown)}
              >
                <span className={`truncate ${selectedSuppliers.length === 0 ? "text-slate-400" : "text-slate-900"}`}>
              ...
              {supplierShowDropdown && (
                <>
                  <div 
                    className="fixed inset-0 z-10" 
                    onClick={() => setSupplierShowDropdown(false)}
                  ></div>
                        ...
                        return (
                          <div
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              if (supplierId) toggleSupplier(supplierId);
                            }}
                          >
                            <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${isSelected ? "bg-primary border-primary text-white" : "border-slate-300"}`}>
