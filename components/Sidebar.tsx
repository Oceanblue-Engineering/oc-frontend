...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const menuItems = [
    ...
    { path: "/lucky-draw", label: t("sidebar.luckyDraw"), icon: Gift },
  ...
  return (
    <>
      ...
      {/* Sidebar Wrapper */}
      <div
        className={`fixed lg:left-0 lg:right-auto max-lg:right-0 max-lg:left-auto top-0 h-screen z-50 py-4 lg:pl-4 max-lg:pr-4 print:hidden flex flex-col transition-all duration-500 ease-in-out ${
          isOpen ? "translate-x-0" : "max-lg:translate-x-full"
        } ${isCollapsed ? "w-20 lg:w-20" : "w-72 lg:w-72"}`}
      >
        <div
          className={`flex-1 bg-white border border-gray-200/70 rounded-3xl shadow-lg flex flex-col overflow-hidden relative justify-between transition-all duration-500 ${
        ...
        <div
          className={`bg-white border border-gray-200/70 rounded-2xl p-2.5 shadow-md flex items-center mt-3 transition-all duration-300 ${
            isCollapsed ? "justify-center" : "justify-between gap-3 px-3"
          }`}
        >
