...
import { bulkLinkSuppliers } from "../services/Inventory/bulkLinkSuppliers";
...
export const Inventory: React.FC = () => {
  ...
  const [transferMode, setTransferMode] = useState<
    "warehouse" | "storefront" | "supplier" | null
  >(null);
  ...
  // Transfer to Supplier State
  const [isTransferSupplierModalOpen, setIsTransferSupplierModalOpen] = useState(false);
  const [suppliersList, setSuppliersList] = useState<any[]>([]);
  const [selectedSupplierId, setSelectedSupplierId] = useState("");
  const [isTransferringToSupplier, setIsTransferringToSupplier] = useState(false);
  ...
  const loadSuppliersList = async () => {
    try {
      ...
      if (response.success && response.data) {
        setSuppliersList(response.data.filter((s: any) => !s.isDeleted));
      }
    } catch (error) {
    ...
    if (editingId) {
      ...
      try {
        ...
        if (formData.suppliers && formData.suppliers.length > 0) {
          apiPayload.suppliers = formData.suppliers;
        }
  ...
  const openEdit = (p: Product) => {
    ...
    setFormData({
      ...
      suppliers: apiProduct?.suppliers || [],
    });
  ...
  const handleOpenTransferSupplierModal = () => {
    if (selectedProductIds.length === 0) {
      toast.error("Please select at least one product to transfer");
      return;
    }
    setIsTransferSupplierModalOpen(true);
  };

  const handleCloseTransferSupplierModal = () => {
    setIsTransferSupplierModalOpen(false);
    setSelectedSupplierId("");
  };

  const handleTransferToSupplier = async () => {
    ...
    setIsTransferringToSupplier(true);
    try {
      const inventoryIds = selectedProductIds
        .map((productId) => {
          const apiProduct = apiProducts.find(
      ...
      const response = await bulkLinkSuppliers({
      ...
      if (response.success) {
        toast.success(
          response.message || "Inventory transferred to supplier successfully",
        );
        setSelectedProductIds([]);
        setShowSelectBoxes(false);
        setTransferMode(null);
        handleCloseTransferSupplierModal();
        await loadProducts();
      } else {
        toast.error(response.message || "Failed to transfer to supplier");
      }
    } catch (error: any) {
      console.error("Error transferring to supplier:", error);
      toast.error(error.message || "Failed to transfer to supplier");
    } finally {
      setIsTransferringToSupplier(false);
    }
  };
    ...
    <div className="w-full lg:h-[calc(100vh-2rem)]">
      <div className="bg-white border border-gray-200/70 rounded-3xl p-6 shadow-md flex flex-col gap-6 lg:h-full lg:overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 border-b border-gray-100 pb-5">
          ...
          <div className="flex flex-row items-center gap-3 overflow-x-auto pb-2 w-full lg:w-auto scrollbar-hide">
            <button
              ...
              <>
                {transferMode === "storefront" && (
                  <button
                    ...
                    className="px-5 py-2 text-sm font-semibold rounded-full bg-orange-600 text-white hover:bg-orange-700 transition-all shadow-md flex items-center gap-2 cursor-pointer whitespace-nowrap shrink-0"
                  >
                    <Store className="w-4 h-4" />
                    <span>
                ...
                {transferMode === "supplier" && (
                  ...
                  >
                    <Truck className="w-4 h-4" />
                    <span>
                      Confirm Supplier ({selectedProductIds.length})
                    </span>
                  </button>
                )}
                <button
                  ...
                  className="px-4 py-2 text-sm font-semibold rounded-full bg-red-500 text-white hover:bg-red-600 transition-all cursor-pointer whitespace-nowrap shrink-0"
                >
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-ocean-500/20 focus:border-ocean-500 transition-all"
          />
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="relative min-w-[220px]">
              <select
                ...
                className="w-full pl-4 pr-10 py-2.5 bg-white border border-gray-200 rounded-2xl text-xs font-medium text-slate-600 focus:outline-none focus:ring-2 focus:ring-ocean-500/20 focus:border-ocean-500 transition-all appearance-none cursor-pointer"
              >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b flex justify-between items-center">
              ...
              <button
                onClick={handleCloseTransferSupplierModal}
            ...
            <div className="p-6 space-y-4">
              ...
              {suppliersList.length === 0 && (
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                  <p className="text-sm text-amber-800">
                    No suppliers available. Please make sure suppliers are loaded or create one first.
                  </p>
                </div>
              )}
