...
export const Purchasing: React.FC = () => {
  ...
  const loadPurchases = async (
    ...
    try {
      const queryParams: any = { page, limit, status };
      ...
      const res = await fetchPurchases(queryParams);
      if (res.success) {
        setPOList(res.data);
  ...
  const loadDeletedPurchases = async (
    ...
    try {
      const queryParams: any = { page, limit, isDeleted: true };
      if (paymentType && paymentType !== "all") {
        queryParams.paymentType = paymentType;
      }
      if (paymentStatus && paymentStatus !== "all") {
      ...
      {activeTab === "po" && (
        <>
          ...
          <PODetailModal
            ...
            suppliers={suppliers}
            onPaymentSuccess={loadPurchases}
          />
        </>
      )}
