...
| `fetchProducts` | GET | `/inventory` | `category?, status?, search?, supplier?, page?, limit?, sortBy?, sortOrder?` | Product[] |
...
| `bulkLinkSuppliers` | POST | `/inventory/bulk-link-suppliers` | `{ productIds[], supplierIds[] }` | `{ matchedCount, modifiedCount }` |
| `bulkUnlinkSuppliers` | POST | `/inventory/bulk-unlink-suppliers` | `{ productIds[], supplierIds[] }` | `{ matchedCount, modifiedCount }` |
